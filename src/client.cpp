...
bool Client::udpStreamRun(sock_t udpSock) {
    ...
    if (!stream_mode_) {
        ...
        std::string cmd = "ffmpeg -hide_banner -loglevel error -i \"" + h264Source_ + "\" -c copy -f h264 - 2>/dev/null";
        LOG_GEN_INFO("Starting ffmpeg to read stream: {}", h264Source_);

        FILE *fp = popen(cmd.c_str(), "r");
        if (!fp) {
            LOG_GEN_ERROR("Failed to launch ffmpeg for stream '{}'", h264Source_);
            return false;
        }
        ...
        LOG_GEN_INFO("Streaming from ffmpeg -> sending to {}:{} (use_fec={})", host_, udpPort_, use_fec_ ? "yes" : "no");
        ...
        while (true) {
            ...
            // If stream is non-looping and external stop requested, break
            if (stop_tcp_listener_.load()) {
                LOG_GEN_INFO("stop requested, breaking stream loop");
                break;
            }
        } // while true reading ffmpeg

        pclose(fp);
        LOG_GEN_INFO("Stream finished (pclosed), exiting udpStreamRun");
        return true;
#endif
    }
}
